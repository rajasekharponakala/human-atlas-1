...
import {conceptMatchesQuery,expandAnatomyQuery} from './anatomy';
...
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');const q=data.query.toLowerCase().trim();const terms=expandAnatomyQuery(q);const byLen=(a:Concept,b:Concept)=>a.name.length-b.name.length;const direct=atlas.concepts.filter(c=>c.name.toLowerCase().includes(q)||c.id.toLowerCase().includes(q)).sort(byLen);const extra=atlas.concepts.filter(c=>!direct.includes(c)&&conceptMatchesQuery(c,terms)).sort(byLen);return [...direct,...extra].slice(0,30).map(c=>({id:c.id,name:c.name,pieces:c.elements.length}));}},
